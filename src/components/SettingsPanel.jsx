...
const SettingsPanel = () => {
  ...
  const validateMerchantguyKey = async () => {
    ...
    if (!merchantguyUrl) {
      setMerchantguyValidationResult({ success: false, message: "Gateway URL is required" });
      return;
    }
    
    setIsValidatingMerchantguy(true);
    try {
      await useStore.getState().validateMerchantguyApiKey(merchantguyApiKey, merchantguyUrl);
      setMerchantguyValidationResult({ success: true, message: 'Merchantguy API key is valid!' });
      // Auto-toggle the app on successful validation
      toggleConnectedApp('merchantguy');
    } catch (error) {
      setMerchantguyValidationResult({ success: false, message: error.message });
    } finally {
  ...
  return (
    <div className="p-6 bg-white">
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">LLM Providers</h2>
        <SettingsMenu />
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="p-6 border border-gray-200 rounded-lg bg-white shadow-sm mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">WooCommerce</h3>
          
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">WooCommerce URL:</label>
            <input
              value={url}
              onChange={(e) => {
          ...
          <div className="flex gap-3 mb-4">
            <button
              ...
              className="px-4 py-2 bg-teal-600 text-white rounded-md text-sm font-medium hover:bg-teal-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
            >
          ...
          {wooValidationResult && (
            <div className={`flex items-center gap-2 p-3 rounded-md text-sm ${wooValidationResult.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {wooValidationResult.success ? <FiCheck /> : <FiAlertCircle />}
        ...
        <div className="p-6 border border-gray-200 rounded-lg bg-white shadow-sm mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Merchantguygateway</h3>
          ...
          {merchantguyValidationResult && (
            <div className={`flex items-center gap-2 p-3 rounded-md text-sm ${merchantguyValidationResult.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {merchantguyValidationResult.success ? <FiCheck /> : <FiAlertCircle />}
        ...
        <div className="p-6 border border-gray-200 rounded-lg bg-white shadow-sm mb-6">
          ...
          {googleValidationResult && (
            <div className={`flex items-center gap-2 p-3 rounded-md text-sm ${googleValidationResult.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {googleValidationResult.success ? <FiCheck /> : <FiAlertCircle />}
        ...
        {/* Zendesk Settings */}
        <div className="p-6 border border-gray-200 rounded-lg bg-white shadow-sm mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Zendesk</h3>
          ...
          {zendeskValidationResult && (
            <div className={`flex items-center gap-2 p-3 rounded-md text-sm ${zendeskValidationResult.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {zendeskValidationResult.success ? <FiCheck /> : <FiAlertCircle />}
        ...
        {/* Ontraport Settings */}
        <div className="p-6 border border-gray-200 rounded-lg bg-white shadow-sm mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Ontraport</h3>
          ...
          {ontraportValidationResult && (
            <div className={`flex items-center gap-2 p-3 rounded-md text-sm ${ontraportValidationResult.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {ontraportValidationResult.success ? <FiCheck /> : <FiAlertCircle />}
